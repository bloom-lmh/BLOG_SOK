...
::: tip 理解
适配器模式的本质是：**把一种不兼容的接口或调用方式，转换成客户端需要的统一形式**。

“给老接口提供一个新接口”只是最常见的应用场景，并不是适配器模式的必要条件。只要不同对象的调用方式不统一，都可以通过适配器进行转换。例如 Spring MVC 的 `HandlerAdapter`，就是把 `HandlerMethod`、`Controller`、`HttpRequestHandler` 等不同处理器的调用方式，统一适配成 `DispatcherServlet` 可以使用的 `handle()` 调用。

对象适配器通常通过组合持有被适配对象，因此适配器也常被称为包装器（Wrapper）。
:::
